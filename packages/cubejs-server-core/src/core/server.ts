  ...
  protected handleConfiguration(opts: CreateOptions): ServerCoreInitializedOptions {
    ...
    const options: ServerCoreInitializedOptions = {
      ...
      driverFactory: (ctx) => {
        ...
        throw new Error(
          `Unexpected return type, dbType must return string, actual: ${getRealType(dbType)}`
        );
      },
      ...
      getDriver: async (dataSource = 'default') => {
        ...
        return driverPromise[dataSource] = (async () => {
          ...
          try {
            ...
            throw new Error(
              `Unexpected return type, driverFactory must return object, actual: ${getRealType(driver)}`
            );
          } catch (e) {
      ...
      getExternalDriverFactory: this.options.externalDriverFactory && (async () => {
        ...
        return externalPreAggregationsDriverPromise = (async () => {
          ...
          try {
            ...
            throw new Error(
              `Unexpected return type, externalDriverFactory must return object, actual: ${getRealType(driver)}`
            );
          } catch (e) {
